...
//						newuser.status = status;
				...
				default:
					...
					if (code == "change") {
						if (name != null) {
							log.debug( "Conference::default::change[" + participantsSO.data[name].userid
									+ "," + participantsSO.data[name].name + "]");
						}
					}			 
					break;
			}
		...
		{
			var event : ConnectedEvent = new ConnectedEvent();
		...
		{
			var event : DisconnectedEvent = new DisconnectedEvent(message);
			dispatcher.dispatchEvent(event);
		}
	}
}