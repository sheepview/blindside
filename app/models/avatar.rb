class Avatar < ActiveRecord::Base
  ...
  def convert
    if ENV["OS"] =~ /Windows/
      ...
      "C:\\ImageMagick-6.3.7-Q16\\convert"
    else
      "/usr/bin/convert"
    end
  end
  ...
  def successful_conversion?
    ...
    # Ensure that the smaill and large images both work by writing to a normal file.
    # (Small file show up as StringIO, larger ones as Tempfiles)
  ...
  def valid_file?
    # The upload should be non-empty.
    if @image.size.zero?
    ...
    if @image.size > 1.megabyte
      errors.add(:image, "can't be bigger that 1 megabyte")
      return false
    end
    return true
  end
