

<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
        <meta name="layout" content="main" />
        <title>Attendee List</title>
    </head>
    <body>
        <div class="nav">
            <span class="menuButton"><a class="home" href="${createLinkTo(dir:'')}">Home</a></span>
            <span class="menuButton"><g:link class="create" action="create">New Attendee</g:link></span>
        </div>
        <div class="body">
            <h1>Attendee List</h1>
            <g:if test="${flash.message}">
            <div class="message">${flash.message}</div>
            </g:if>
            <div class="list">
                <table>
                    <thead>
                        <tr>
                   	        <th>Conference</th>
                   	    
                   	        <g:sortableColumn property="name" title="Name" />
                        
                   	        <g:sortableColumn property="emailAddress" title="Email Address" />
                        
                   	        <g:sortableColumn property="dateCreated" title="Date Created" />
                        
                   	        <g:sortableColumn property="lastUpdated" title="Last Updated" />
                        
                        </tr>
                    </thead>
                    <tbody>
                    <g:each in="${attendeeList}" status="i" var="attendee">
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                            <td>${attendee.conference?.encodeAsHTML()}</td>
                        
                            <td><g:link action="show" id="${attendee.id}">${attendee.name?.encodeAsHTML()}</g:link></td>
                        
                            <td>${attendee.emailAddress?.encodeAsHTML()}</td>
                        
                            <td>${attendee.dateCreated?.encodeAsHTML()}</td>
                        
                            <td>${attendee.lastUpdated?.encodeAsHTML()}</td>
                        
                        </tr>
                    </g:each>
                    </tbody>
                </table>
            </div>
            <div class="paginateButtons">
                <g:paginate total="${Attendee.count()}" />
            </div>
        </div>
    </body>
</html>
